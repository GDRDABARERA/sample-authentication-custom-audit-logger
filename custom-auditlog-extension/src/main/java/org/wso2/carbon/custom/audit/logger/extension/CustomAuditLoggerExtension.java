...
package org.wso2.carbon.custom.audit.logger.extension;
...
import static org.wso2.carbon.identity.core.util.IdentityUtil.getFirstIP;

/**
 * A sample provided for custom logging in the authentication flow.
 * But this will not applicable for management console login or token grant types.
 */
public abstract class CustomAuditLoggerExtension extends AbstractAuthenticationDataPublisher {

    private static final Log log = LogFactory.getLog(CustomAuditLoggerExtension.class);
    ...
    public void publishAuthenticationStepSuccess(HttpServletRequest request, AuthenticationContext context,
        ...
        // Extra-parameters to identify the user.

        authenticationData.addParameter(USER_AGENT,request.getHeader(USER_AGENT));
        authenticationData.addParameter(HOST, request.getHeader(HOST));
        authenticationData.addParameter(COOKIE, request.getHeader(COOKIE));
        authenticationData.addParameter(REFERER, request.getHeader(REFERER));
        authenticationData.addParameter(CLIENT_IP, getClientIpAddress(request));
    ...
    public void publishAuthenticationStepFailure(HttpServletRequest request, AuthenticationContext context,
        ...
        authenticationData.setStepNo(step);
//        // Extra-parameters to identify the user in accesslogs

        authenticationData.addParameter(USER_AGENT,request.getHeader(USER_AGENT));
        authenticationData.addParameter(HOST, request.getHeader(HOST));
        authenticationData.addParameter(COOKIE, request.getHeader(COOKIE));
        authenticationData.addParameter(REFERER, request.getHeader(REFERER)); // Should publish the event to both SP tenant domain and the tenant domain of the user who did the login
        authenticationData.addParameter(CLIENT_IP, getClientIpAddress(request));
        // attempt
...
//        // Extra parameters to identify the user
    ...
    public void publishSessionCreation(HttpServletRequest request, AuthenticationContext context, SessionContext
        ...
        sessionData.addParameter(USER_AGENT,request.getHeader(USER_AGENT));
        sessionData.addParameter(HOST, request.getHeader(HOST));
        sessionData.addParameter(COOKIE, request.getHeader(COOKIE));
        sessionData.addParameter(REFERER, request.getHeader(REFERER));
        sessionData.addParameter(CLIENT_IP, getClientIpAddress(request));
